...
namespace JustLib.UnitViews
{
    public partial class UserListBox : UserControl
    {
        ...
        public void AddUser(IUser friend)
        {
            ...
            ChatListSubItem subItem = new ChatListSubItem(friend.ID, "", friend.Name + "(" + friend.ID + ")", friend.UserStatus == UserStatus.Online ? "온라인" : "오프라인", this.ConvertUserStatus(friend.UserStatus), this.resourceGetter.GetHeadImage(friend));
            subItem.Tag = friend;
        ...
        {           
            ...
            items[0].PersonalMsg = user.UserStatus == UserStatus.Online ? "온라인" : "오프라인";
        ...
        public ChatListBox _ChatListBox 
        {
            get { return this.chatListBox; } 
        }
        ...
        public void AddCatalog(string catelogName)
        {
            ...
            foreach (ChatListItem item in this.chatListBox.Items)
            {
                ...
                {
                    MessageBox.Show("같은 이름의 그룹이 이미 존재합니다.");
                    return;
                }
            }
        ...
        private void 删除分组ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show(string.Format("그룹 [{0}] 은 표준그룹입니다. 삭제하실수 없습니다.", name));
                return;
            }

            if (this.chatListBox.SelectItem.SubItems.Count > 0)
            {
                MessageBox.Show(string.Format("그룹 [{0}] 그룹은 적어도 한개이상은 존재하여야 합니다." ,name));
                return;
            }

            if (MessageBox.Show(string.Format("정말로 그룹 [{0}] 을 삭제하시겠습니까?", name), "메시지", MessageBoxButtons.YesNo) == DialogResult.No)
            {
                return;
            }
